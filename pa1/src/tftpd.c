...
int main(int argc, char *argv[])
{
	...
	while(true){
		...
		while(true){
			...
			if(ntohs(ack.opC) == ACK_OPC && ack.blockNr == nextBlock){
				nextBlock++;
				isReading = true;
			...
			else if(ntohs(ack.opC) != ACK_OPC || ack.blockNr != nextBlock){
				err.opC = htons(ERR_OPC);
				strcpy(err.errMsg, "Acknowledgement error\n");
